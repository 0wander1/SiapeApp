...
type ProductoEditarScreenNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  'ProductoEditarScreen'
>;

type ProductoEditarScreenRouteProp = RouteProp<
  RootStackParamList,
  'ProductoEditarScreen'
>;
...
function ProductoEditarScreen() {
  const navigation = useNavigation<ProductoEditarScreenNavigationProp>();
  const route = useRoute<ProductoEditarScreenRouteProp>();
  ...
  useEffect(() => {
    const fetchProducto = async () => {
      ...
      try {
        ...
        setValorNeto(
          producto.valor_neto != null ? String(producto.valor_neto) : '',
        );
        setValorDeVenta(
          producto.valor_de_venta != null
            ? String(producto.valor_de_venta)
            : '',
        );
        ...
        setBodegaIdActual(producto.bodega_id_bodega ?? null);
      } catch (err) {
  ...
  const handleGuardar = async () => {
    ...
    if (!nombreProducto.trim()) {
      setErrorGuardar('Ingresa el nombre del producto.');
      return;
    }
    ...
    if (!valorNeto.trim() || Number.isNaN(valorNetoNum)) {
      setErrorGuardar('Ingresa un valor neto válido.');
      return;
    }
    ...
    if (!valorDeVenta.trim() || Number.isNaN(valorDeVentaNum)) {
      setErrorGuardar('Ingresa un valor de venta válido.');
      return;
    }
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response?.data?.message) {
        ...
        setErrorGuardar('Ocurrió un error al guardar el producto.');
      }
    } finally {
      setGuardando(false);
    }
...
export default ProductoEditarScreen;
